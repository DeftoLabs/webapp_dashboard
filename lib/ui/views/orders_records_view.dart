            ...
            Container(
              ...
              child: Consumer<OrdenesProvider>(
                builder: (context, ordenesProvider, child) {
                  ...
                  List<String> usuariosZona = ordenesProvider.ordenes
                      .map((orden) => orden.ruta.first.usuarioZona.nombre)
                      .toSet()
                      .toList();
                          ...
                          value: value, child: Text(value));
                                       ...
                                       child: Consumer<OrdenesProvider>(
                                         builder: (context, ordenesProvider, child) {
                                           ...
                                           // Obtener usuarios sin duplicados
                                           List<String> clientes = ordenesProvider.ordenes
                                           .where((orden) => orden.clientes.isNotEmpty)
                                           .map((orden)=> orden.clientes.first.nombre)
                                           .toSet()
                                           .toList();
              ...
              children: [
                PaginatedDataTable(
                  ...
                  columns: [
                   ...
                   DataColumn(label: const Text('Date'), onSort: (colIndex, _) {
                      ordenesProvider.sortColumnIndex = colIndex;
                      ordenesProvider.sort<DateTime>((ordenes) => ordenes.fechacreado);
                    }),
                    DataColumn(label: const Text('Customer'), onSort: (colIndex, _) {
                      ordenesProvider.sortColumnIndex = colIndex;
                      ordenesProvider.sort<String>((ordenes) => 
                        ordenes.clientes.isNotEmpty ? ordenes.clientes.first.nombre : ''
                      );
                    }),
                    DataColumn(label: const Text('Total'), onSort: (colIndex, _) {
                      ordenesProvider.sortColumnIndex = colIndex;
                      ordenesProvider.sort((ordenes) => ordenes.total);
                    }),
                    DataColumn(label: const Text('Status'), onSort: (colIndex, _) {
                      ordenesProvider.sortColumnIndex = colIndex;
                      ordenesProvider.sort<String>((ordenes) => ordenes.status);
                    }),
                    DataColumn(label: const Text('Sales'), onSort: (colIndex, _) {
